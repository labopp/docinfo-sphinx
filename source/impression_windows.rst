...
#. Il faut alors rechercher l'imprimante voulue en indiquant son adresse IP.
   Pour cela, à l'écran suivant, choisir la méthode ``Ajouter une imprimante à
   l'aide d'une adresse TCP/IP ou un nom d'hôte`` puis cliquer sur
   ``Suivant``.
...
#. La dernière étape consiste à installer le pilote associé à l'imprimante. Il
   est possible que ce pilote ne soit pas encore installé auquel cas Windows
   demandera de le choisir dans une liste et/ou de le rechercher sur Internet.
   Dans un tel cas, il faut mentionner le nom du constructeur et le type
   d'imprimante (voir la :ref:`liste des imprimantes <ListeImprimantes>` du
   laboratoire).
